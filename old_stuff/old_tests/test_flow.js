	...
	describe('hydration (detail = 7)', function () {
		...
		describe("#toJSON", function(){
			it('should be able to export itself as JSON', function(done){
				...
				fs.writeFile(__dirname + '/JSON/F7.json', json, 'utf8', function(){
					...
					zlib.gzip(json, function(err, data){
						fs.writeFile(__dirname + '/JSON/F7.json.gz', data, 'utf8', done)
					});
				})
			...
			it('should be able to export its index as JSON', function(done){
				...
				var fs = require('fs');
				fs.writeFile(__dirname + '/JSON/F7.index.json', json, 'utf8', function(){
					console.log('created, wrote index JSON in %s ms', new Date().getTime() - time);
					var zlib = require('zlib');
					zlib.gzip(json, function(err, data){
						fs.writeFile(__dirname + '/JSON/F7.index.json.gz', data, 'utf8', done)
					});
				})
			...
			it('should be able to export its index as JSON -- readable', function(done){
				...
				var fs = require('fs');
				fs.writeFile(__dirname + '/JSON/F7.index.formatted.json', json, 'utf8', function(){
					console.log('created, wrote index JSON in %s ms', new Date().getTime() - time);
					done();
				});
			});
