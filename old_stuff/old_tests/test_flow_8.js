...
describe('Planet', function () {
	...
	describe('hydration (detail = 8)', function () {

		var planet;

		before(function () {
			var time = new Date().getTime();
			planet = new Planet(1, 8, {});
			console.log('planet created: %s seconds, %s points ', Math.floor( (new Date().getTime() - time)/1000), planet.get_uvs().length);

			var data = planet.vertices_by_detail();
			_.each(data, function(item){
				console.log('detail: %s, vertices: %s', item.detail, item.vertices.length);
			})
		});
		...
		describe('#set_elevation', function () {
			...
			it.skip('should be able to render elevations', function (done) {
				var time = new Date().getTime();
				...
				var props = {
					...
					width:        360 * SCALE,
					height:       180 * SCALE,
					file:         __dirname + '/render/heightF8.png',
		...
		describe("#toJSON", function(){
			it('should be able to export itself as JSON', function(done){
				...
				var fs = require('fs');
				fs.writeFile(__dirname + '/JSON/F8.json', json, 'utf8', function(){
					console.log('created, wrote JSON in %s ms', new Date().getTime() - time);
					var zlib = require('zlib');
					zlib.gzip(json, function(err, data){
						fs.writeFile(__dirname + '/JSON/F8.json.gz', data, 'utf8', done)
					});
				})
			...
			it('should be able to export itself as JSON -- readable', function(done){
				...
				var fs = require('fs');
				fs.writeFile(__dirname + '/JSON/F8.formatted.json', json, 'utf8', function(){
					console.log('created, wrote JSON in %s ms', new Date().getTime() - time);
					done();
				});
			});
			it('should be able to export its index as JSON', function(done){
				var time = new Date().getTime();
				var json = JSON.stringify(planet.index.toJSON());
				var fs = require('fs');
				fs.writeFile(__dirname + '/JSON/F8.index.json', json, 'utf8', function(){
					console.log('created, wrote index JSON in %s ms', new Date().getTime() - time);
					var zlib = require('zlib');
					zlib.gzip(json, function(err, data){
						fs.writeFile(__dirname + '/JSON/F8.index.json.gz', data, 'utf8', done)
					});
				})

			});

			it('should be able to export its index as JSON -- readable', function(done){
				var time = new Date().getTime();
				var json = JSON.stringify(planet.index.toJSON(), true, 1);
				var fs = require('fs');
				fs.writeFile(__dirname + '/JSON/F8.index.formatted.json', json, 'utf8', function(){
					console.log('created, wrote index JSON in %s ms', new Date().getTime() - time);
					done();
				});
			});
