...
var readline = require('./../line_reader');
...
var GLOBE_DATA = path.resolve(__dirname, '../../../GLOBE_data');
...
var biome_data = require('./../../../GLOBE_data/biomes.json');
...
Biome.make_legend = function (file, callback) {
    ...
    canvas_to_file(canvas, file, function () {
        console.log('done writing %s', file);
        callback();
    });
};
        ...
        this.planet.vertices(function (vertex) {
            if (!vertex.data('main_biome')) {
                ...
                if (neighbor_biomes.length > 0) {
                    console.log('filling vertex %s', vertex.index);
                    console.log('... with %s', neighbor_biomes);
