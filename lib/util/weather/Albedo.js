...
assert(path.existsSync(ALBEDO_FILE), 'cannot find albedo file');
var readline = require('line-by-line');
var lat_lon_to_vertex = require('./../lat_lon_to_Vector3');
var Planet = require('./../../Planet');

var _DEBUG = false;

console.log('readline: -------------------------------- %s', util.inspect(readline));
...
_.extend(Albedo.prototype, {
    ...
    load: function (callback) {
        ...
        index.init(function () {
            ...
            var tasks = self.planet.vertices().length;

            self.planet.vertices(function (vertex) {
                var l = gate.latch();
                var task = tasks;
    ...
    load_albedo: function (callback) { */
        var self = this;
        console.log('done reading elevation');
        var handle = new readline(ALBEDO_FILE);
    ...
    read_line: function (line) {
        ...
        var point3 = lat_lon_to_vertex(lat, lon - 180, true);
